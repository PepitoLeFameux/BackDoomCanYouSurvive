...
void Ennemi::DeathSequence()
{
    ...
    *ennemisRestants -= 1;
    ...
    float probas[] = {30.0f, 4.0f, 5.0f, 10.0f};
...
void Ennemi::Render()
{
    if(itemState)
    {
        decal = 0.07f*sin(PI*(GetTime() - deathTime));
        DrawBillboard(*camera, itemStateTexture, Vector3Add(position, (Vector3){0.0f, decal, 0.0f}), 0.2f, WHITE);
        DrawCylinder((Vector3){position.x, 0.0f, position.z}, 0.1f, 0.1f, 0.0001f, 16, DARKGRAY);
    }
    else if(!dead)
    {
...
void Ennemi::Action()
{
    ...
    Vector3 cameraPos = (*camera).position;
    distJoueur = sqrt(pow(position.x - cameraPos.x, 2) + pow(position.z - cameraPos.z, 2));
    if((VisionDirecte(position) && distJoueur < 7) || dmged) poursuite = true;
