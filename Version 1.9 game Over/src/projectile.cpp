...
void Projectile::Init(Ennemi *listeEnnemis, int nbEnnemis, Color *mapCouleurs, Texture2D dimensionsMap,
                      ...
                      Texture2D *texturesRoquette, Camera *camera, int largeur, int hauteur)
{
    ...
    Projectile::nbEnnemis = nbEnnemis;
...
void Projectile::Action()
{
    if(active)
    {
        ...
        if(hit)
        {
            if(!detonated){Detonate(); detonated = true;}
